...
export class ReceivedMessageModalComponent implements OnInit {
  ...
  ngOnInit() {
    this.modalManageService.activeModalsData.subscribe((m) => {
  ...
  public send() {
    if (this.modalData.answer) {
      swal.fire(
        "Wysłanie wiadomości",
        "Wiadomość została już nadana wcześniej",
        "error"
      );
    } else if (this.message.invalid) {
      swal.fire(
        "Wysłanie wiadomośći",
        "Pole z wiadomością nie może być puste",
        "error"
      );
    } else {
      ...
      const { email, detail } = this.layoutManageService.adminsData
        .getValue()
        .find((a) => a.email === emailAdmin);
