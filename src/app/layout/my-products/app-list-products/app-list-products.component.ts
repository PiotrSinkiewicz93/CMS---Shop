...
export class AppListProductsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.productManageService.imagesData.subscribe(
  ...
  public filterData(): void {
    ...
    this.time = setTimeout(() => {
      this.limit = 5;
      const productsManage = this.productManageService.productsData
        .getValue()
        .slice();
      if (this.sortState === 1) {
  ...
  public async deleteImages(): Promise<any> {
    ...
    const filPrs = this.products.filter(
      (pr) => pr.ref !== this.productDelete.ref
    );
