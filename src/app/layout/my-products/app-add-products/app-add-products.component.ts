...
export class AppAddProductsComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.subsProducts = this.productsManageService.productsData.subscribe(
      (prs) => {
        this.products = prs;
    ...
    this.productsManageService.categoryData.subscribe(
      (cat) => (this.categories = cat)
    );
    this.subsStatusAddProduct =
      this.productsManageService.addProductData.subscribe(
        this.statusAddProduct.bind(this)
      );
  }

  ngOnDestroy(): void {
    this.productsManageService.addProductData.next({
      status: StatusProduct.Idle,
    });
  ...
  public statusAddProduct(response: AddProduct): void {
    if (response.status === StatusProduct.Complete) {
      ...
      this.initForm();
      // tslint:disable-next-line:no-string-literal
      document.getElementsByClassName("image-select")[0]["style"]["display"] =
        "none";
    }
  }
  ...
  public async updateDataProduct(): Promise<any> {
    ...
    this.editProduct = {
      ...
      reviewsCustomer: this.editProduct.reviewsCustomer
        ? this.editProduct.reviewsCustomer
        : null,
    };

    if (this.FLAG_CHANGE_PICTURE) {
      this.FLAG_CHANGE_PICTURE = false;
      const refStorage = this.firebaseService.firebase
        .storage()
        .ref("PRODUCTS/" + this.editProduct.image.name);
      await refStorage.put(image);
      const url = await this.firebaseService.firebase
        .storage()
        .ref("PRODUCTS/" + this.editProduct.image.name)
        .getDownloadURL();
    ...
    await this.firebaseService
      .getDataBaseRef("products")
      .child(indexPr.toString())
      .set(this.editProduct);
    swal.fire(
      "Aktualizacja Produktu",
      "Produkt został zaaktualizowany",
      "success"
    );
  }
