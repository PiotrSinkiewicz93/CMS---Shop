...
export class AppNewMessagesComponent implements OnInit {
  ...
  public findAdmin(): void {
    ...
    this.admin = this.layoutManageService.adminsData
      .getValue()
  ...
  public async sendDefaultMessage(): Promise<any> {
    ...
    if (result.value) {
      ...
      await swal.fire(
        title,
        "Wiadomość została wysłana. Wiadomość zostanie przeniesiona do przeczytanych",
        "success"
      );
      this.resetMessage();
    } else {
  ...
  public async deleteMessage(): Promise<any> {
    ...
    if (result.value) {
      const messages = this.messagesManageService.messageData
        .getValue()
        .filter((msg) => msg.id !== this.targetMsg.id);
  ...
  public async markMessage(): Promise<any> {
    ...
    const text =
      "Czy jesteś pewny, że chcesz oznaczyć wiadomość jako przeczytaną?";
    const type = "warning";
    const result = await this.dialogConfirm(title, text, type);
    ...
    if (result.value) {
      ...
      await swal.fire(
        title,
        "Wiadomość została oznaczona jako przeczytana. Wiadomość zostanie przeniesiona do przeczytanych",
        "success"
      );
      this.resetMessage();
    } else {
      swal.fire(
        title,
        "Wiadomość nie została oznaczona jako przeczytana",
        "info"
      );
    }
  }
  ...
  public prevSelect(): void {
    if (this.selectValue > 1) {
      const index = this.cpMessages.findIndex(
        (msg) => msg.id === this.targetMsg.id
      );
