...
export class AppLayoutComponent implements OnInit {
  ...
  public onResize(): void {
    if (window.innerWidth < 768 && this.vissibleLeftPanel) {
  ...
  ngOnInit(): void {
    ...
    this.ordersManageService.ordersData.subscribe((orders) => {
      if (orders) {
  ...
  public tryLogin() {
    const { email, password } = JSON.parse(
      localStorage.getItem("shop-admin")
    ) as Admin;
  ...
  public setNewDataLogin(): void {
    fetch("https://api.ipify.org?format=json")
      ...
      .then((data) => {
        ...
        this.user.logins
          ? this.user.logins.push(login)
          : (this.user.logins = [login]);
  ...
  public get sumNotification(): number {
    ...
    return (
      (notif.messages || 0) + (notif.evaluations || 0) + (notif.reviews || 0)
    );
  }
