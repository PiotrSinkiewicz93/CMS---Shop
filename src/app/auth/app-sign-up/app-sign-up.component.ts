...
export class AppSignUpComponent implements OnInit {
      ...
      this.firebaseService.firebase
        ...
        .catch((error) => {
          if (error.code === "auth/email-already-in-use") {
            swal.fire(
              "Rejestracja",
              "Profil o takim emailu już istnieje!",
              "error"
            );
          } else {
  ...
  public saveToStorage(): void {
    ...
    localStorage.setItem("shop-admin", JSON.stringify({ email, password }));
  }

  public setVisiblePassword(refPassword): void {
    this.vissiblePassword = !this.vissiblePassword;
    this.vissiblePassword
      ? (refPassword.type = "text")
      : (refPassword.type = "password");
  }

  public setVisibleRepeatPassword(refRepeatPassword): void {
    this.vissibleRepeatPassword = !this.vissibleRepeatPassword;
    this.vissibleRepeatPassword
      ? (refRepeatPassword.type = "text")
      : (refRepeatPassword.type = "password");
  }
  ...
  public checkValidateRepeatPassword(): boolean {
    ...
    if (control.errors) {
      if (
        (control.errors.required || control.errors.equalTo) &&
        control.touched
      ) {
        return true;
      } else {
